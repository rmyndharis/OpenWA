...
class StatusResource:
    ...
    def list(self, session_id: str) -> dict[str, list[StatusRecord]]:
        return self._http.request("GET", f"/api/sessions/{session_id}/status")

    def from_contact(self, session_id: str, contact_id: str) -> dict[str, list[StatusRecord]]:
        return self._http.request("GET", f"/api/sessions/{session_id}/status/{contact_id}")

    def send_text(self, session_id: str, body: SendTextStatusRequest) -> StatusRecord:
        return self._http.request("POST", f"/api/sessions/{session_id}/status/send-text", body=body)

    def send_image(self, session_id: str, body: SendImageStatusRequest) -> StatusRecord:
        return self._http.request("POST", f"/api/sessions/{session_id}/status/send-image", body=body)

    def send_video(self, session_id: str, body: SendVideoStatusRequest) -> StatusRecord:
        return self._http.request("POST", f"/api/sessions/{session_id}/status/send-video", body=body)

    def delete(self, session_id: str, status_id: str) -> None:
        self._http.request("DELETE", f"/api/sessions/{session_id}/status/{status_id}")
