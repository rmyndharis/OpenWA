...
// 2. Local user-managed configuration (does not override real process env)
if (fs.existsSync(localEnvPath)) {
  dotenv.config({ path: localEnvPath, override: false });
}

// 3. User-managed .env (does not override real process env)
if (fs.existsSync(userEnvPath)) {
  console.log('[Bootstrap] Loading .env from:', userEnvPath);
  dotenv.config({ path: userEnvPath, override: false });
}

// 4. Dashboard-saved config (does not override .env or process env)
if (fs.existsSync(generatedEnvPath)) {
  ...
  const minimalConfig = `# Aeon WhatsAPP API Configuration
...
async function bootstrap() {
  ...
  const config = new DocumentBuilder()
    .setTitle('Aeon WhatsAPP API')
    .setDescription('Hosted WhatsApp automation API for Aeon platform workspaces')
    .setVersion('0.1.6')
  ...
  console.log(`Aeon WhatsAPP API is running on: http://localhost:${port}`);
