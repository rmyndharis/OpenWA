...
// Request/response shape for the shared "does this already exist" + identity resolution endpoint
// every automatic writer (auto-tag, Import from Chats) should use instead of its own
// create-if-missing logic — see docs/32 §5.
...
export const clientMappingApi = {
  ...
  // docs/32 §5. Used by bulk import instead of create() so the exact bug fixed once in the backend
  // (the same real contact ending up with two jids) can't resurface here.
