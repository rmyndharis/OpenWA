...
> **Call events and call rejection are Baileys only.** On current WhatsApp Web builds whatsapp-web.js no longer detects a ringing call. A whatsapp-web.js session emits no `call.*` event, so it never has a `callId` to reject and `autoRejectCalls` has no effect. `POST .../calls/link` works on both engines.
...
| `call.received`                                   | An incoming voice/video call starts ringing. **Baileys only** in practice: on current WhatsApp Web builds whatsapp-web.js no longer detects a ringing call                                                                            | `{ callId, from, isVideo, isGroup, timestamp }` — `callId` is the id to pass to `POST /sessions/:sessionId/calls/:callId/reject`                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
