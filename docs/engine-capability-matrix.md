...
> **Phantom support.** The drift gate's throw-heuristic cannot see adapter methods that silently stub (return `null`/`[]` + a warn log) without throwing. Five wwjs entries — `getCatalog`, `getProducts`, `getProduct`, `getContactStatus`, `getContactStatuses` — are marked `not-available` in the matrix even though their adapter bodies do not throw, because the library either has no API (catalog) or the adapter stubs instead of calling the available symbol (status read). When the drift gate is extended to assert against this matrix, it must consult `status`, not just the throw pattern, for these rows (or the adapter stubs must start throwing).
...
| Method | baileys | wwjs |
|---|---|---|
| `getContactStatus` | not-available — **library-limitation** | not-available — **adapter-gap** |
| `getContactStatuses` | not-available — **library-limitation** | not-available — **adapter-gap** |

- **`getContactStatus` / `getContactStatuses` (wwjs, adapter-gap).** The library HAS the getters — `Client.getBroadcastById(contactId)` (`index.d.ts:136`; `Client.js:2771`) and `Client.getBroadcasts()` (`index.d.ts:133`; `Client.js:2759` → `WWebJS.getAllStatuses`). Each `Broadcast.msgs: Message[]`. The adapter stubs to `[]` without calling them (`whatsapp-web-js.adapter.ts:1732,1740`). Wiring: `getBroadcasts()` then flatten — for each `Broadcast`, map `b.msgs` to `Status[]` with the contact resolved via `broadcast.getContact()`.
